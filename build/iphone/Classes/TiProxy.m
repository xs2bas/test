...
//Rare exceptions to indicate a bug in the _huurders code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_huurders = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_huurders]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.huurders.module"
