...
Alloy.Globals.addFetchErrorHandler = function(collection, options) {
    ...
    options.error = function(collection, e) {
        ...
        original_error_callback && original_error_callback();
    };
...
Alloy.Globals.removeLoginData = function() {
    ...
    for (i = 0; i < reset_collections.length; i++) Alloy.createCollection(reset_collections[i]).reset();
};
...
Alloy.Globals.httpRequestAuthenticated = function(url, options) {
    var client = Ti.Network.createHTTPClient({
        onload: function(e) {
            options.onSuccess && (options.returnData ? options.onSuccess(this.responseData) : options.onSuccess(this.responseText));
        },
        onerror: function(e) {
            options.onError && options.onError(this.responseText);
        },
        timeout: 5e3
    });
    ...
    options.postData ? client.send(options.postData) : client.send();
};

Alloy.Globals.appendRowToSection = function(tableView, row, sectionIndex) {
    var sections = tableView.data;
    if (!sections || sections.length < sectionIndex || isNaN(+sectionIndex)) return;
...
Alloy.Globals.moneyFormat = function(amount) {
    ...
    -1 != amount.indexOf(",") && (amount = amount.substr(0, amount.indexOf(",") + 3));
    return "€ " + amount;
};
...
Alloy.Globals.getCollectionUrl = function(collection) {
    var controller_url = collection.adapter.collection_name.replace(/(?:^|\.?)([A-Z])/g, function(x, y) {
...
Alloy.Globals.getSetting = function(name, fetchOptions) {
    ...
    if ((setting = settings.where({
        name: name
    })) && setting[0]) return setting[0].get("value");
    return null;
};

Alloy.Globals.checkConnection = function(online_callback, offline_callback) {
    if (!Titanium.Network.online) return Alloy.Globals.toggleConnection(false, online_callback, offline_callback);
    Alloy.Globals.initializeSettings(function() {
        Alloy.Globals.toggleConnection(1 == Alloy.Globals.getSetting("modules_app_enabled"), online_callback, offline_callback);
    });
};
...
Alloy.Globals.toggleConnection = function(toggle, online_callback, offline_callback) {
    ...
    true == toggle ? online_callback(toggle !== previous_is_online) : offline_callback(toggle !== previous_is_online);
};
